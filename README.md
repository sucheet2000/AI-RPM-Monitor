# 🫀 AI-RPM Monitor

**AI-Powered Remote Patient Monitoring System**

A full-stack real-time patient vitals monitoring platform featuring edge AI classification, Kafka event streaming, and an interactive React dashboard.

![Tech Stack](https://img.shields.io/badge/Python-3.9+-blue?logo=python)
![React](https://img.shields.io/badge/React-18-61DAFB?logo=react)
![Kafka](https://img.shields.io/badge/Kafka-Streaming-231F20?logo=apachekafka)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-15-4169E1?logo=postgresql)
![Docker](https://img.shields.io/badge/Docker-Compose-2496ED?logo=docker)

---

## 📋 Table of Contents

- [Overview](#-overview)
- [Architecture](#-architecture)
- [Quick Start](#-quick-start)
- [Running the Services](#-running-the-services)
- [API Endpoints](#-api-endpoints)
- [Running Tests](#-running-tests)
- [Project Structure](#-project-structure)

---

## 🎯 Overview

AI-RPM Monitor is a full-stack, real-time platform designed to revolutionize remote patient monitoring (RPM) through intelligent, edge-based AI classification and triage.

### The Problem

In traditional RPM, clinical staff are often overwhelmed by large volumes of raw, unclassified vital sign data. This data noise leads to **alert fatigue** and significantly **delays response times** when a true critical event occurs.

### The Solution: AI-Powered Clinical Triage

AI-RPM Monitor solves this by applying **real-time, intelligent triage** at the data source. The system filters noise, identifies anomalies with an AI classifier (Isolation Forest), and uses the Gemini LLM to generate **instant, actionable clinical summaries**. This approach significantly accelerates the time from critical event detection to clinical intervention.

**Core Workflow:**
1. **Edge devices** capture vitals and send them to the producer.
2. **AI classifier** categorizes readings as Normal/Warning/Critical.
3. **Kafka** streams classified data in real-time.
4. **Flask API** consumes, persists data to PostgreSQL, and calls the LLM on critical events.
5. **React Dashboard** provides true live visualization and critical alerts.

---

## 🏗 Architecture

```
┌─────────────────┐     ┌─────────────┐     ┌──────────────┐
│  Edge Producer  │────▶│    Kafka    │────▶│   Consumer   │
│  (Python + ML)  │     │   (vitals)  │     │  (Flask API) │
└─────────────────┘     └─────────────┘     └──────┬───────┘
                                                   │
                                                   ▼
┌─────────────────┐                        ┌──────────────┐
│ React Dashboard │◀───────────────────────│  PostgreSQL  │
│   (Tailwind)    │      REST API          │   Database   │
└─────────────────┘                        └──────────────┘
```

---

## 🚀 Quick Start
## 🚀 Deployment

The entire system (Infrastructure + Backend + Frontend + Data Producer) is containerized with Docker.

### Prerequisites

-   **Docker & Docker Compose** (v2.0+)
-   **Gemini API Key** (for AI Triage)

### 1. Configure Environment

```bash
cp .env.example .env
```

### 4. Start the Application Services

You will need **3 separate terminal windows**:

**Terminal 1: Consumer Service (Back-end)**
```bash
pip install -r requirements.txt
python -m consumer.app
```
*Runs on http://localhost:5001*

**Terminal 2: Vitals Producer (Data Simulator)**
```bash
python vitals_producer.py
```
*Generates real-time synthetic patient data*

**Terminal 3: Frontend Dashboard**
```bash
cd frontend
npm install
npm start
```
*Runs on http://localhost:3000*

---

## 🖥 Using the Dashboard

1.  Open **http://localhost:3000**
2.  Select a patient from the list (e.g., `patient-demo001` if generated).
3.  **Real-time Vitals:** Watch the Live Heart Rate chart update instantly via WebSockets.
4.  **Critical Alerts:** If the producer generates a critical value, a **Red Alert Banner** will appear.
5.  **AI Triage:** Click the banner to view the **Structured JSON Report** generated by Gemini.
6.  **Wellness Report:** Review the "Patient Wellness Report" card for the plain-text clinical summary.

---

## 🔧 Troubleshooting

**Port Conflicts?**
- Kafka needs port `9093` (external) and `9092` (internal).
- Backend runs on `5001` (to avoid macOS Control Center on 5000).

**"Connection Error" in Dashboard?**
- Ensure the Consumer API is running in Terminal 1.
- Check browser console for network errors.

**No LLM Summaries?**
- Verify `GEMINI_API_KEY` is set in `.env`.
- Check Consumer logs for "🤖 LLM Triage completed".

---

## 📡 API Endpoints

Base URL: `http://localhost:5001`

| Endpoint | Method | Description |
|----------|--------|-------------|
| `/` | GET | Health check |
| `/api/vitals/<patient_id>` | GET | Last 24h of vitals |
| `/api/alerts/<patient_id>` | GET | Critical alerts only |
| `/api/summaries/<patient_id>` | GET | LLM wellness summaries |
| `/api/stats` | GET | System-wide statistics |

### Example Requests

```bash
# Get patient vitals (last 24 hours)
curl http://localhost:5001/api/vitals/patient-abc123

# Get critical alerts
curl http://localhost:5001/api/alerts/patient-abc123

# Get system stats
curl http://localhost:5001/api/stats
```

---

## 🧪 Running Tests

### Run All Tests

```bash
pytest -v
```

### Run Specific Test Suites

```bash
# Test the AI classifier
pytest tests/test_classifier.py -v

# Test the API endpoints
pytest tests/test_api.py -v

# Test Flask routes
pytest tests/test_routes.py -v
```

### Expected Output

```
========================= test session starts =========================
collected 51 items

tests/test_classifier.py::TestEdgeClassifier::test_normal_reading PASSED
tests/test_classifier.py::TestSyntheticCriticalData::test_critical_low_spo2 PASSED
tests/test_api.py::TestGetVitalsEndpoint::test_get_vitals_returns_200 PASSED
...
========================= 51 passed in 33.00s =========================
```

---

## 📁 Project Structure

```
AI-RPM-Monitor/
├── docker-compose.yml       # Infrastructure (Postgres, Kafka, Zookeeper)
├── requirements.txt         # Python dependencies
├── vitals_producer.py       # Kafka producer with AI classifier
├── edge_classifier.py       # scikit-learn anomaly detection
├── run.py                   # Flask app entry point
│
├── app/                     # Original Flask app
│   ├── __init__.py
│   ├── routes.py
│   └── models.py
│
├── consumer/                # Kafka consumer service
│   ├── app.py               # Flask API server
│   ├── models.py            # VitalsRecord, LLMSummary tables
│   └── kafka_consumer.py    # Robust Kafka consumer
│
├── frontend/                # React + Tailwind dashboard
│   ├── package.json
│   ├── tailwind.config.js
│   └── src/
│       ├── App.jsx
│       ├── components/
│       │   ├── VitalsChart.jsx
│       │   ├── AlertBanner.jsx
│       │   ├── WellnessCard.jsx
│       │   └── Modal.jsx
│       ├── pages/
│       │   ├── HomePage.jsx
│       │   └── PatientDashboard.jsx
│       └── services/
│           └── api.js
│
└── tests/                   # pytest test suites
    ├── test_classifier.py   # 31 classifier tests
    ├── test_api.py          # 20 API endpoint tests
    └── test_routes.py       # Basic route tests
```

---

## 🎨 Dashboard Features

- **Real-time Vitals Chart** — Interactive time-series visualization with Recharts
- **Critical Alert Banner** — Sticky red banner for immediate attention
- **LLM Summary Modal** — Click alerts to view raw JSON clinician data
- **Wellness Report Card** — AI-generated patient analysis
- **Real-time Streaming** — Data is pushed instantly via WebSockets, eliminating polling latency for true live monitoring.

---

## 🛠 Technology Stack

| Layer | Technology |
|-------|------------|
| **Frontend** | React 18, Tailwind CSS, Recharts, React Router |
| **Backend** | Flask, Flask-SQLAlchemy, confluent-kafka |
| **AI/ML** | scikit-learn (Isolation Forest) |
| **Database** | PostgreSQL 15 |
| **Messaging** | Apache Kafka |
| **Infrastructure** | Docker Compose |
| **Testing** | pytest, pytest-flask |

---

## 📜 License

MIT License — See [LICENSE](LICENSE) for details.

---

<div align="center">
  <sub></sub>
</div>
